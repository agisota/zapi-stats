...
export function generateUserProfile(entry: LeaderboardEntry, allEntries: LeaderboardEntry[]): UserProfile {
  ...
  if (entry.requestsPerDay > 500 && entry.uniqueModels > 30) {
    ...
    summary = 'Специалист по премиум-моделям с фокусом на качество';
  ...
  if (entry.outputRatio > 0.05) patterns.push(`Высокая доля выходных токенов (${(entry.outputRatio * 100).toFixed(1)}%) — генерирует много контента`);
  else if (entry.outputRatio < 0.005) patterns.push(`Минимальная доля выходных токенов (${(entry.outputRatio * 100).toFixed(2)}%) — агентный/инструментальный режим`);
  else patterns.push(`Стандартная доля выходных токенов (${(entry.outputRatio * 100).toFixed(2)}%)`);
  ...
  if (entry.tokensCacheRead > 0) {
    ...
    patterns.push(`Использует кэш промптов — ${(cacheRatio * 100).toFixed(1)}% попаданий в кэш`);
  ...
  if (entry.tokensReasoning > 0) {
    patterns.push(`Использует модели с рассуждением — ${entry.tokensReasoning.toLocaleString('ru-RU')} токенов рассуждения`);
  }
  ...
  if (inputCostRatio > 0.9) patterns.push(`Затраты на вход — ${(inputCostRatio * 100).toFixed(0)}% стоимости приходится на входные токены (большие промпты)`);
  else if (inputCostRatio < 0.5) patterns.push(`Затраты на выход — ${((1 - inputCostRatio) * 100).toFixed(0)}% стоимости приходится на генерацию ответов`);
  ...
  if (entry.uniqueProviders === 1 && entry.requests > 100) antiPatterns.push(`Зависимость от одного провайдера — все запросы идут через него`);
  if (entry.avgLatency > 5000) antiPatterns.push(`Очень высокая задержка (${(entry.avgLatency / 1000).toFixed(1)} с) — снижает продуктивность`);
  if (entry.tokensCacheRead === 0 && entry.tokensIn > 10_000_000) antiPatterns.push(`Упущенная экономия — ${(entry.tokensIn / 1_000_000).toFixed(0)} млн входных токенов без кэширования`);
  ...
  if (entry.avgLatency < 500 && entry.requests > 50) pros.push(`Быстрые ответы — ${entry.avgLatency} мс средняя задержка`);
  ...
  if (entry.tokensCacheRead > 0) pros.push(`Использует кэширование промптов — экономит на повторных запросах`);
  ...
  if (entry.tokensReasoning > 0) pros.push(`Использует модели с рассуждением — подходит для глубокого анализа`);
  if (entry.longestSessionMessages > 500 && entry.successRate > 0.9) pros.push(`Устойчивые длинные сессии — высокая успешность даже при марафонном использовании`);

  // --- CONS ---

  if (entry.successRate < 0.9) cons.push(`Высокий процент ошибок (${((1 - entry.successRate) * 100).toFixed(1)}%)`);
  if (entry.avgLatency > 2000) cons.push(`Высокая задержка — ${(entry.avgLatency / 1000).toFixed(1)} с в среднем`);
  ...
  if (entry.outputRatio < 0.003) cons.push(`Крайне низкий выход — возможно, не получает полезных ответов`);
  if (entry.avgTtft > 5000) cons.push(`Долгое ожидание первого токена (${(entry.avgTtft / 1000).toFixed(1)} с) — снижает интерактивность`);
  ...
  if (entry.successRate < 0.8) recommendations.push(`Проверьте лимиты: ${entry.errorCount} ошибок могут быть из-за ограничений API-провайдера`);
  if (entry.avgLatency > 3000) recommendations.push(`Снизьте задержку: попробуйте Groq или Cerebras для задач, не требующих глубокого рассуждения`);
  ...
  if (entry.tokensCacheRead === 0 && entry.requests > 100) recommendations.push(`Включите кэширование промптов — это может сэкономить до 90% на повторных запросах`);
  if (entry.outputRatio < 0.003) recommendations.push(`Проверьте выход: крайне низкий объем ответа может указывать на проблемы с промптами`);
  ...
  if (entry.successRate === bestSuccessRate && entry.requests > 50) highlights.push('Наивысшая успешность запросов');
