...
export function createApp(db: Database, logsPathOrOptions?: string | CreateAppOptions, accountDbOrAnonSecret?: Database | string) {
  const accountDb = typeof accountDbOrAnonSecret === 'string' ? undefined : accountDbOrAnonSecret;
  const injectedAnonSecret = typeof accountDbOrAnonSecret === 'string' ? accountDbOrAnonSecret : undefined;
  ...
  const anonymizer = new Anonymizer(options.anonSecret ?? injectedAnonSecret ?? resolveAnonSecret());
