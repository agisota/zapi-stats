...
describe('account billing and DV.net webhook', () => {
  ...
  test('tracks skill activations and per-user expense analytics', async () => {
    ...
    db.prepare(`
      ...
      VALUES ('log-skill-1', ?, '/skills/autopilot/run', 200, 'gpt-5.4', 'codex', ?, 1000, 1000, 100)
    `).run(observedAt, body.data.defaultKey.gatewayName);
    db.prepare(`
      INSERT INTO usage_history (provider, model, api_key_id, api_key_name, tokens_input, tokens_output, success, status, timestamp)
      VALUES ('codex', 'gpt-5.4', ?, ?, 1000000, 100000, 1, 'ok', ?)
    `).run(body.data.defaultKey.gatewayKeyId, body.data.defaultKey.gatewayName, observedAt);
